...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_sql = "INSERT INTO Users (username, email, password, role, security_question, security_answer) 
                 VALUES ('$username', '$email', '$password', 'Admin', '$security_question', '$security_answer')";
    
    if ($conn->query($user_sql)) {
        $user_id = $conn->insert_id;
        
        // Insert into Admins table
        $admin_sql = "INSERT INTO Admins (admin_id) VALUES ($user_id)";
        
        if ($conn->query($admin_sql)) {
            echo json_encode(['success' => true, 'message' => 'Admin added successfully.']);
        } else {
            // Rollback user insertion if admin insertion fails
