...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_sql = "INSERT INTO Users (username, email, password, role, security_question, security_answer) 
                VALUES ('$username', '$email', '$password', 'Customer', '$security_question', '$security_answer')";
    
    if ($conn->query($user_sql)) {
        $user_id = $conn->insert_id;
        
        // Insert into Customers table
        $customer_sql = "INSERT INTO Customers (customer_id) VALUES ($user_id)";
        
        if ($conn->query($customer_sql)) {
            echo json_encode(['success' => true, 'message' => 'Customer added successfully.']);
        } else {
            // Rollback user insertion if customer insertion fails
